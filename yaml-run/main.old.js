...
function printStatsSummary(totalTime) {
    console.log('\n--- Execution Summary ---');
...
function executeShell(command, envVars) {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            ...
            executionStats.push({ type: 'CMD', name: cleanCommand, duration, status });
...
async function runTask(taskName, scriptConfig, variables) {
    ...
    try {
        ...
        const duration = Date.now() - start;
        executionStats.push({ type: 'TASK', name: taskName, duration, status });
    }
}
