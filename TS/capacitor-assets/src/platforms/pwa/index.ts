...
import { basename, extname, isAbsolute, join, posix, relative, sep } from 'path';
...
// Determine destination directory for generated PWA assets
// If pwaIconsPath is provided, treat it as an exact path relative to project root (or absolute if given).
// If pwaOutRoot is provided and pwaIconsPath is relative, prefix it physically with that root (projectRoot/pwaOutRoot/...).
function resolveIconsDestDir(
  options: AssetGeneratorOptions,
  projectRoot: string,
  pwaRootForFallback: string,
  pwaAssetsDirForFallback: string,
): string {
  const iconsPath = options.pwaIconsPath?.trim();
  const outRoot = options.pwaOutRoot?.trim();

  if (iconsPath && iconsPath.length > 0) {
    if (isAbsolute(iconsPath)) {
      return iconsPath;
    }
    // Relative path: optionally prefix with outRoot physically
    if (outRoot && outRoot.length > 0) {
      return join(projectRoot, outRoot, iconsPath);
    }
    return join(projectRoot, iconsPath);
  }

  // Fallback to legacy behavior: <assetsDir>/icons or <pwaRoot>/icons
  const base = pwaAssetsDirForFallback || pwaRootForFallback;
  return join(base, PWA_ASSET_PATH);
}
...
export class PwaAssetGenerator extends AssetGenerator {
  ...
  const destDir = resolveIconsDestDir(this.options, project.directory ?? '', pwaDir, pwaAssetDir);
    try {
  ...
  const pwaDir = await this.getPWADirectory(project.directory ?? undefined);
    ...
    const generatedAssets = await Promise.all(
      icons.map(async (icon) => {
        const assetsDir = await this.getPWAAssetsDirectory(pwaDir);
        const destDir = resolveIconsDestDir(this.options, project.directory ?? '', pwaDir, assetsDir);
        try {
          await mkdirp(destDir);
        } catch {
          // ignore error
  ...
  const pwaAssetDir = await this.getPWAAssetsDirectory(pwaDir);
    ...
    for (const asset of pwaAssets) {
      ...
      // Manifest URL should always reflect pwaIconsPath (without pwaOutRoot), when provided
      let relativePath: string;
      if (this.options.pwaIconsPath) {
        const urlPath = posix.join(this.options.pwaIconsPath.split(sep).join(posix.sep), fname);
        relativePath = urlPath;
      } else {
        const destDir = resolveIconsDestDir(this.options, project.directory ?? '', pwaDir, pwaAssetDir);
        relativePath = relative(pwaDir, join(destDir, fname));
      }
